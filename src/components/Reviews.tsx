...
function Review({imgSrc,className,...props}:ReviewProps){
  ...
  return (
    <div className={
      cn('animate-fade-in rounded-[2.25rem] bg-white p-6 opacity-0 shadow-xl shadow-slate-900/5',className)
    }
...
function ReviewGrid() {
  ...
  const isInView = useInView(containerRef,{once:true,amount:.4})
  ...
  return (
    ...
    >
      ...
      reviewClassName={(reviewIndex)=>
        reviewIndex >= column2.length ? 'lg:hodden': ''
      }
      msPerPixel={10}
      />
      <ReviewColum 
      ...
      <div className='pointer-events-none absolute inset-x-0 bottom-0 h-32 bg-gradient-to-b from-slate-100'/>
    </div>
  )
}
